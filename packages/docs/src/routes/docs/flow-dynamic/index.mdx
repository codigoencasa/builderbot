...
const flowString = addKeyword('ver categorías')
...
const flowString = addKeyword('ver categorías')
    .addAnswer('Estas son las categorías disponibles:', null, async (ctx, {flowDynamic}) => {
        await flowDynamic('Enviar un mensaje texto')
        ...
        await flowDynamic({body:'También puedes enviar un mensaje de esta manera'})
